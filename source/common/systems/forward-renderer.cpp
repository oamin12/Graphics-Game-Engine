...
namespace our {
    ...
    void ForwardRenderer::initialize(glm::ivec2 windowSize, const nlohmann::json& config){
        ...
        if(config.contains("sky")){
            ...
            skyPipelineState.faceCulling.culledFace = GL_FRONT; //because we are drawing the sphere from the inside
    ...
    void ForwardRenderer::render(World* world){
        ...
        std::sort(transparentCommands.begin(), transparentCommands.end(), [cameraForward](const RenderCommand& first, const RenderCommand& second){
            ...
            //we used dot product to determine which object is closer to the camera
            if(dot(first.center, cameraForward) > dot(second.center, cameraForward))
                return true;
